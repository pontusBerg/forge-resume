  ...
  return dateIndexes
    .map((dateIndex, index) => {
      ...
      const bulletEndIndex =
        nextDateIndex === undefined ? cleanedLines.length : Math.max(dateIndex + 1, nextDateIndex - 2)
...
function parseEducation(lines: string[]): Education[] {
  ...
  while (index < cleanedLines.length) {
    ...
    const dateRange = line.match(/\b((?:19|20)\d{2})\s*(?:-|–|—|\bto\b)\s*((?:19|20)\d{2}|Present|Current)\b/i)

    if (dateRange) {
      const school = line.slice(0, dateRange.index).trim().replace(/[-–—,]+$/, "").trim()
